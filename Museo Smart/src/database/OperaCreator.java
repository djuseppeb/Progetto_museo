...
public class OperaCreator implements Creator{
	...
	public boolean updateRoom(int ID, String room ) { //method to modify room
	    Connection conn = DbAccess.getAccess().conn;	 //get Db istance
	    try {
			Statement stmt = conn.createStatement();
	...
	public boolean updateMovement(int ID, String mov ) { //method to modify art movement
	    ...
	    try {
			Statement stmt = conn.createStatement();
			if(mov == "Astrattismo" || mov == "Cubismo" || mov == "Espressionismo" || mov == "Futurismo" || mov == "Futurismo" || mov == "Realismo" || mov == "Arte rinascimentale" ) {
